...
#lemma(name: [$gamma$-contraction])[
  ...
  $cal(T)$ is a *$gamma$-contraction* under $||dot||_oo$ where $||dot||_oo := max_(x in(dot)) x$, 
  which means $||cal(T) f - cal(T) f'||_oo <= gamma||f - f'||_oo $
]
...
Then, we can prove the convergence of VI:
...
#proof(name: "Convergence of VI")[
  ...
  $ =>||f_k - Q^* ||_oo <= gamma^k||f_(k - 1) - Q^* ||_oo "where" gamma in(0, 1) $
]
